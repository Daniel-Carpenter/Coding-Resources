...
public class Main {
	public static void main(String[] args) throws IOException {
		 ...
		 * all are related to each other. Section has been created to give you a break
		 * from a continuous long discussion. When we want any output from a method, we
		 * printed the intended output immediately above the line of code. Looking at
		 * the sample output (SampleOutput.txt) will give you a better idea about the
		 * output format.
		 * 
		 * Section 1
		 */
		...
		/**
		 * 
		 * First, read the text file: Dates.txt; to create hashmap, we will use this
		 * dates.
		 * 
		 * The contents are like: 05.05.2017 11.12.2018
		 * 
		 * 1 In the first part of the printed line is for a year is a leap year or not.
		 * In the second part of the line, calculate the difference between today and
		 * the given date. Today means the server date during execution.
		 * 
		 * 2 At the same time, save these date in a Hashmap as HashMap<LocalDate,
		 * Integer> Keys are in the text file, for map value use numeric values starting
		 * from 1.
		 * 
		 * 3 Output format is: (for format, use the sample output text file, that would
		 * be easier) 2017 is not a leap year, and Difference: 3 years, 5 months, and 9
		 * days. 2014 is not a leap year, and Difference: 6 years, 7 months, and 16
		 * days.
		 * 
		 */
		 ...
		 * Parse the file and store in a hashmap. The dates as the key and as a date
		 * format, not string For example, HashMap<LocalDate, Integer>, but you can use
		 * any date function format of the date will be: 2020-12-31
		 * 
		 * For this section, you have to sort using an algorithm (such as Quick sort,
		 * Bubble sort, or any other sorting algorithm you prefer) Don't use any
		 * APT/built-in function/methods/treemap/ArrayList etc., you have to implement
		 * the algorithm You are free to choose any algorithm you prefer to implement
		 * such as Quick sort, Bubble sort, Radix sort, Insertion sort etc. All these
		 * are for learning purpose only. So, if you use any API instead of own sorting
		 * algorithm, you will lose some points during code review.
		 */
